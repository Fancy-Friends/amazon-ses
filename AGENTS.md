...
npm run provider -- amazon-ses            # regenerate this repo
npm run provider -- amazon-ses --check    # exit 1 on any difference
```
...
| `packages/ui` | `@particle-academy/amazon-ses-ui` — the authoring surface, React, every host | Generated |
| `packages/js` | `@particle-academy/amazon-ses-js` — Node, on `@particle-academy/fancy-connector-core` | Generated |
| `packages/php` | `particle-academy/amazon-ses-php` — PHP 8.4, on `particle-academy/fancy-connector-core` | Generated |
| `packages/py` | `fancy-amazon-ses` — Python 3.11+, stdlib only | Generated |

The authoritative copy of `provider/` lives in the weaver envelope at
`providers/amazon-ses/provider/`. The copy here is emitted from it and `--check`
...
| | Namespace |
|---|---|
| This repo | `github.com/Fancy-Friends/amazon-ses` |
| npm | `@particle-academy/amazon-ses-ui`, `@particle-academy/amazon-ses-js` |
| Packagist | `particle-academy/amazon-ses-php` |
| PyPI | `fancy-amazon-ses` |
...
**Default to plain HTTP.** A Amazon SES SDK is third-party code and is subject to
